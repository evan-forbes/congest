...
func (n *Network) SyncAddValidator(name, region, ip, payloadRoot string) {
	n.AddValidator(name, ip, payloadRoot, region)
}
...
func (n *Network) InitNodes(rootDir string) error {
	...
	for i, v := range vals {
		...
		fmt.Println("wrote config file", filepath.Join(rootDir, vname, "config.toml"), cmtcfg.Instrumentation.TracingTables)
